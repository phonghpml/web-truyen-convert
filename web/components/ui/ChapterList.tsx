  ...
  return (
    <div className="mt-12 w-full">
      <div className="flex justify-between items-end mb-6 border-l-4 border-orange-600 pl-4">
        <h3 className="text-xl font-black uppercase italic text-white">Danh sách chương</h3>
        <button 
          onClick={() => setIsAscending(!isAscending)}
          className="flex items-center gap-2 px-3 py-1.5 bg-gray-900 border border-gray-800 rounded-md text-[11px] text-gray-400 hover:text-orange-500 transition-all uppercase"
        >
          <ArrowUpDown size={14} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 h-[600px] overflow-y-auto pr-4 custom-scrollbar">
        {sortedChapters.map((ch, index) => (
          ...
          >
            <span className="text-[13px] text-gray-300 group-hover:text-white font-serif italic truncate">
              {ch.title_vi}
            </span>
          </button>
        ))}
